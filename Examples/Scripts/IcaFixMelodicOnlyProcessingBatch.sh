	...
	for Subject in ${Subjlist}; do
		...
		if [ -z "${ConcatNames}" ]; then
			...
			for fMRIName in ${fMRINamesFlat}; do
				...
				cmd=("${queuing_command[@]}" "${HCPPIPEDIR}/ICAFIX/hcp_fix_MelodicOnly" "${InputFile}" ${bandpass} ${domot} "${SRTrainingData}" ${FixThreshold} "${DeleteIntermediates}")
		    ...
		    do
				...
				cmd=("${queuing_command[@]}" "${HCPPIPEDIR}/ICAFIX/hcp_fix_multi_run_MelodicOnly" --fmri-names="${InputFile}" --high-pass=${bandpass} --concat-fmri-name="${ConcatFileName}" --motion-regression=${domot} --training-file="${MRTrainingData}" --fix-threshold=${FixThreshold} --delete-intermediates="${DeleteIntermediates}" --config="$config" --processing-mode="$processingmode")
				echo "About to run: ${cmd[*]}"
				"${cmd[@]}"
			done
