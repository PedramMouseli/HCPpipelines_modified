...
if [ -z "${FSL_FIX_CIFTIRW}" ]; then
	FSL_FIX_CIFTIRW='/Users/moayedilab/Documents/HCP_pipelines/HCPpipelines-4.8.0/global/matlab/cifti-matlab';
fi
if [ -z "${FSL_FIX_WBC}" ]; then
	...
	if [ -z "${FSL_FIX_WBC}" ]; then
		...
		WBENCH="/Applications/workbench/"
		if [ "${FSL_FIX_OS}" = "Darwin" ]; then
			FSL_FIX_WBC="${WBENCH}/bin_macosxub/wb_command"
